  ...
  function initLibrary(){
    ...
    function defaultIdeas(){
      ...
      const presets = [
        {
          cat:"Daily drivers",
          title:"Daily starter — get unstuck",
          tags:"#daily #starter",
          model:"Any",
...
[PASTE]`
        ...
        {
          cat:"Writing",
          title:"Professional email (clean + friendly)",
          tags:"#writing #email",
          model:"Any",
...
[PASTE]`
        ...
        {
          cat:"Coding",
          title:"Debug buddy — give me the fix",
          tags:"#coding #debug",
          model:"Any",
...
[PASTE]`
        ...
        {
          cat:"Visual",
          title:"Image prompt vs video prompt (contrast)",
...
[PASTE]`
        ...
        {
          cat:"Creator",
          title:"TikTok hook generator (12–15 sec)",
          tags:"#creator #tiktok",
          model:"Any",
...
[PASTE]`
        ...
        {
          cat:"Research / OSINT",
          title:"Fast research plan (sources first)",
          tags:"#research #osint",
          model:"Any",
...
Keep it grounded.`
  ...
  // ---------------------------
  // Ideas (starter prompts) tab
  // ---------------------------
  function initIdeas(){
    const list = q("#ideasList");
    const search = q("#ideasSearch");
    const catSel = q("#ideasCat");
    const onlyFav = q("#ideasFav");
    if(!list) return;

    // Seed if empty (safe: only writes once per browser)
    const seeded = storeGet(STORE_KEYS.librarySeeded, false);
    let ideas = storeGet(STORE_KEYS.ideas, []);
    if(!ideas || !ideas.length){
      ...
      storeSet(STORE_KEYS.ideas, ideas);

      if(!seeded){ storeSet(STORE_KEYS.librarySeeded, true); }
    }

    function collectCats(items){
      const cats = Array.from(new Set(items.map(x=>x.cat||"Other"))).sort((a,b)=>a.localeCompare(b));
      const opts = ['<option value="All">All</option>'].concat(cats.map(c=>`<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`));
      if(catSel) catSel.innerHTML = opts.join("");
    }

    function matchQuery(item, q){
      if(!q) return true;
      const hay = [item.title, item.tags, item.model, item.notes, item.prompt, item.cat].join(" ").toLowerCase();
      return hay.includes(q);
    }

    function renderList(){
      ideas = storeGet(STORE_KEYS.ideas, []);
      const qv = (search?.value||"").trim().toLowerCase();
      const cat = (catSel?.value||"All");
      const favOnly = !!onlyFav?.checked;

      let rows = ideas.slice().sort((a,b)=> (b.created||0)-(a.created||0));
      rows = rows.filter(it=> matchQuery(it,qv));
      if(cat && cat!=="All") rows = rows.filter(it=>(it.cat||"Other")===cat);
      if(favOnly) rows = rows.filter(it=>!!it.fav);

      if(!rows.length){
        list.innerHTML = `<div class="lib__empty">No ideas match your filters.</div>`;
        return;
      }

      list.innerHTML = rows.map(it=>{
        return `
          <div class="lib__card" data-id="${it.id}">
            <div class="lib__head">
              <div class="lib__title">${escapeHtml(it.title||"Untitled")}</div>
              <div class="lib__actions">
                <button class="btn btn--ghost" data-act="star">${it.fav ? "★" : "☆"}</button>
                <button class="btn btn--ghost" data-act="copy">Copy</button>
                <button class="btn btn--ghost" data-act="save">Save to Library</button>
                <button class="btn btn--ghost" data-act="send">Send to Buddy</button>
              </div>
            </div>
            <div class="lib__meta">
              <span>${new Date(it.created||Date.now()).toLocaleString()}</span>
              <span class="lib__pill">${escapeHtml(it.cat||"Other")}</span>
              ${it.model ? `<span class="lib__pill">${escapeHtml(it.model)}</span>` : ""}
              ${it.tags ? `<span class="lib__tags">${escapeHtml(it.tags)}</span>` : ""}
            </div>
            ${it.notes ? `<div class="lib__notes">${escapeHtml(it.notes)}</div>` : ""}
            <pre class="lib__pre lib__pre--scroll">${escapeHtml(it.prompt||"")}</pre>
          </div>
        `;
      }).join("");
    }
    ...
    list.addEventListener("click", async (e)=>{
      ...
      if(!btn) return;
      const card = e.target.closest(".lib__card");
      ...
      if(act==="send"){
        ...
        location.hash = "buddy";
        render("buddy");
        setStatus("Sent to Buddy.");
        return;
      }

      if(act==="save"){
        const lib = storeGet(STORE_KEYS.library, []);
        const newItem = {
        ...
        lib.unshift(newItem);
        storeSet(STORE_KEYS.library, lib);
